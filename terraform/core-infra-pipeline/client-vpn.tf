...
## Codebuild Role
#############################################################################################
module "vpn_module_build_step_role" {
  source           = "../../modules/iam-role"
  role_name        = "terraform-vpn-module-build-step-role-${var.namespace}-${var.environment}"
  role_description = "terraform vpn module build step role"
  principals = {
  ...
  policy_name        = "terraform-vpn-module-build-step-policy-${var.namespace}-${var.environment}"
  policy_description = "terraform vpn module build step role"
...
module "vpn_module_build_step_codebuild_project" {
  ...
  name                              = "terraform-vpn-module-build-step-code-build-${var.namespace}-${var.environment}"
  description                       = "terraform vpn module build step module code build project"
  build_timeout                     = 480
  queued_timeout                    = 480
  service_role                      = module.vpn_module_build_step_role.arn
