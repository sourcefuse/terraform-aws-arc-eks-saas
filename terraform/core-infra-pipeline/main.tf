...
module "deployment_pipeline" {
  ...
  stages = [
    ...
    { name = "Opensearch-Ops", category = "Build", owner = "AWS", provider = "CodeBuild", input_artifacts = "source_output", output_artifacts = "", run_order = 4, project_name = "${aws_codebuild_project.os_ops_module_build_step_codebuild_project.name}" },
