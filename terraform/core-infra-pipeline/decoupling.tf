...
## Codebuild Role
#############################################################################################
module "decoupling_module_build_step_role" {
  source           = "../../modules/iam-role"
  role_name        = "terraform-decoupling-role-${var.namespace}-${var.environment}"
  role_description = "terraform decoupling module build step role"
  principals = {
  ...
  policy_name        = "terraform-decoupling-policy-${var.namespace}-${var.environment}"
  policy_description = "terraform decoupling module build step policy"
...
module "decoupling_module_build_step_codebuild_project" {
  ...
  name                              = "terraform-decoupling-module-build-step-code-build-${var.namespace}-${var.environment}"
  description                       = "terraform decoupling build step module code build project"
  build_timeout                     = 480
  queued_timeout                    = 480
  service_role                      = module.decoupling_module_build_step_role.arn
