...
# if [ -d "../output" ]; then
#     cp -r ../output/* onboarded-tenants/bridge/application/ || { echo "Failed to copy files"; exit 1; }
# else
#     echo "'output' folder does not exist. Skipping file copy."
# fi

if [ -d "../output/cognito" ]; then
    cp -r ../output/cognito/* onboarded-tenants/bridge/application/cognito/ || { echo "Failed to copy cognito files"; exit 1; }
else
    echo "'cognito' folder does not exist. Skipping cognito file copy."
fi

if [ -d "../output/auth0" ]; then
    cp -r ../output/auth0/* onboarded-tenants/bridge/application/auth0/ || { echo "Failed to copy auth0 files"; exit 1; }
else
    echo "'auth0' folder does not exist. Skipping auth0 file copy."
fi

if [ -d "../output/keycloak" ]; then
    cp -r ../output/keycloak/* onboarded-tenants/bridge/application/keycloak/ || { echo "Failed to copy keycloak files"; exit 1; }
else
    echo "'keycloak' folder does not exist. Skipping keycloak file copy."
fi
# Copy tenant specific tfvars and config file to repository
cp -r ../*.tfvars onboarded-tenants/bridge/infra/terraform/ || { echo "Failed to copy files"; exit 1; }

cp -r ../*.hcl onboarded-tenants/bridge/infra/terraform/ || { echo "Failed to copy files"; exit 1; }

# Copy Pooled tfvars and config file to repository
cp -r ../pool-infra/*.hcl onboarded-tenants/bridge/infra/terraform/pool-infra/ || { echo "Failed to copy files"; exit 1; }

cp -r ../pool-infra/*.tfvars onboarded-tenants/bridge/infra/terraform/pool-infra/ || { echo "Failed to copy files"; exit 1; }
